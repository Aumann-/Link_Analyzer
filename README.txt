...
If using a crawled file, a 3D tree is generated showing each link in relation to each other link. Multiple options are available for this file type.
	...
	*Export as OBJ - Export the current 3D model as a OBJ file for use in a 3D modeling program such as Blender. This allows the models to be edited or scaled for later export to be sent to a 3D printer. This feature is slow to export so do not close the render window or the application for a few minutes after clicking the button.